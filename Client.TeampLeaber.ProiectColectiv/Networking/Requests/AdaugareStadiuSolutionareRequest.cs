...
{
    public class AdaugareStadiuSolutionareRequest : BaseRequest
    {
        private StadiuSolutionareModel _stadiu;

        public AdaugareStadiuSolutionareRequest(StadiuSolutionareModel stadiu)
        {
            _stadiu = stadiu;
        }
        ...
        internal async Task<bool> Run()
        {
            try
            {
                response = await this.PutAsJsonAsync(Constants.StadiiSolutionarePath, _stadiu);
